...
KISSY.add(function (S) {
    ...
    return {
        ...
        isInAlipay: function() {
            var that = this;
            return window.alipay;
        },

        isInAlipayNewClient: function() {
            //8.0钱包中公众帐号也有该对象，但不支持pushWindow，fuck!
            var match = navigator.userAgent.match(/AlipayClient[\s\/][\d\.]+/igm);
            var version = match ? parseInt(match[0].match(/[\d\.]+/igm)[0].split('.').join('')) : 0;
            version = parseInt(version.toString().substr(0, 3));
            return window.AlipayJSBridge && version >= 810;
        },

        pageGoto: function(href) {
            var that = this;
            if(that.isInAlipayNewClient()){
                href = that.absoultePath(href);
                AlipayJSBridge.call('pushWindow', {
                    ...
                    param: {
                        ...
                        backBehavior: "back" // 返回
                    }
                });
            }else if(that.isInAlipay()) {
                href = that.absoultePath(href);
                alipay.navigation.pushWindow(href);
            }else{
